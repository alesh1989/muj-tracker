          ...
          <>
            ...
            {(() => {
              ...
              divTx.forEach(t=>{
                ...
                byMonthAndTicker[key][t.ticker]=(byMonthAndTicker[key][t.ticker]||0)+(t.dividendAmount||0);
              });
            ...
            <div style={S.card}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
                {(lang==="en"?T.en.months:T.cs.months).map((monthName,mi)=>{
                  const now3=new Date();
                  const received2=activeTransactions.filter(t=>t.type==="dividend"&&new Date(t.date).getFullYear()===divCalYear&&new Date(t.date).getMonth()===mi).reduce((s,t)=>s+(t.dividendAmount||0),0);
                    ...
                    <svg viewBox={`0 0 ${w2} ${h2}`} style={{width:"100%",height:"auto"}}>
                      {[0,0.25,0.5,0.75,1].map(t3=>(
                        <g key={t3}>
                          ...
                          <text x={pad2.l-4} y={pad2.t+iH2*t3+4} textAnchor="end" fill="#475569" fontSize="8">{(maxV2*(1-t3)/1e6).toFixed(1)}M</text>
                        </g>
                      ))}
                      ...
                      {[0,5,10,15,20,25,30].map(y=>(
                        <text key={y} x={xS2(y)} y={h2-4} textAnchor="middle" fill="#475569" fontSize="8">{y}r</text>
                      ))}
                    </svg>
                  );
                })()}
